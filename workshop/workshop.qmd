...
<!-- This one usage of map_int might not be worth the purrr dependency -->
...
```{r}
boys |> 
  mutate(region = if_else(reg == "city", "city", "suburbs")) |> 
  ggmice(aes(age, hgt)) + 
  geom_point() + 
...
:::{.callout-note title="Multiply imputed repeated analyses statistics" collapse=true}
...
Related packages we didn't have time for that may be of interest:
...
* [CCB Office Hours](https://dbmi.hms.harvard.edu/about-dbmi/core-computational-biomedicine/office-hours)
  * [Link to book time](https://outlook.office.com/bookwithme/user/c43422598df4487cb77662221798fec7%40hms.harvard.edu?anonymous)
