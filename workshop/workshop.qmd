...
We'll first need to download the data from GitHub. For completeness, here's the (long) script used to generate this data with some Bioconductor packages. You don't need to run this.
...
It contains two data frames and a vector of gene names. There's the original data `dat` and the same data with NAs `na_dat`. `to_imp` is a list of 20 genes whose expression values have dropouts. The data contains a few thousand cells and 500 highly variable genes: 
