...
This is simply done with the `mice()` function. Given that we saw that the default imputation method for height didn't look so great, let's change it to Bayesian linear regression for age and height and leave it at the defaults for age and region: 
