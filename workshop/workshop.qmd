...
Now let's try a missing data analysis of the `boys` dataset with `mice`. Let's assume our field of study is obesity.
...
This is simply done with the `mice()` function. Given that we saw that the default imputation method for height didn't look so great, let's change it to Bayesian linear regression for age and height and leave it at the defaults for age and region. Let's also set the RNG seed and drop BMI as discussed:
...
It looks a little better for the younger boys who were imputed as too short before. It doesn't pick up the non-linearity of the trend and now seems to be undershooting in the middle, but it's centered overall at least.
...
Looks fine. In practice you'd want to sanity check the other imputed variables as well, but let's move on.
...
Obtaining similar estimates from the complete cases makes sense: we have over 700 complete cases and relatively few with NAs. The information gain from a handful of incomplete cases is negligible in this instance. But of course, 
:::
...
In this section we'll look at a synthetic example to see how well imputation can "rescue" some dropped out value. For the sake of brevity, rather than doing differential expression or some complicated analysis, we'll just examine the original, dropped-out, and imputed datasets with UMAPs to see how they compare.

For the sake of simplicity, we'll assume that we've used some upstream tool that is able to label "suspicious" zeros and replace them with NAs.

Here's the (long) script used to generate this data with some Bioconductor packages. You don't need to run this.
...
Basically it identifies the 20 most highly expressed genes in the pyramidal CA1 cell type, and replaces 20% of the observations in those cells with NA. The NAs are introduced completely at random, which is actually a fairly reasonable missingness model in the context of single cell barcode dropout.
...
url = "https://github.com/ccb-hms/missing_data/raw/refs/heads/main/workshop/sc.RData"
...
It contains the original data `dat` and the same data with NAs knocked in `na_dat`. `to_imp` is a list of 20 genes with knocked in NAs. The data contains a few thousand cells and 500 highly variable genes: 
...
na_umap = na_dat |> 
  ...
  mutate(l1c = na_dat |> na.omit() |> pull(l1c))
...
You can see the shape of most of the blobs has stayed the same, but the pyramidal CA1 cells have all but vanished. 

Now let's run a single imputation. There are many fancier ways to plausibly do this, but let's just use `mice`. **This will take a minute** since there are 20 genes with missing values and so many predictors for each one.
...
`mice` doesn't like to encourage working with the inner content of a `mids` object, but just this once:
...
Pretty good! This is not a rigorous comparison of the imputed against original values, but it seems to have recovered that green blob pretty well.

:::{.callout-note title="Exercise"}

Try comparing the imputed values for Plp1 against the true values. Are they any good?
