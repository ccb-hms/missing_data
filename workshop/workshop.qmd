...
:::{.callout-tip title="ggplot themes" collapse=true}
...
Deciding which zeros are biological versus technical is a fairly difficult statistical problem that we are going to ignore. For the sake of simplicity, we'll assume that we've used some upstream tool that has accurately labelled "suspicious" zeros and replaced them with NAs.
...
Basically it identifies the 20 most highly expressed genes in the pyramidal CA1 cell type in the classic Zeisel Brain Data, and replaces 20% of the observations in those cells with NA. The NAs are introduced completely at random, which is actually a fairly reasonable missingness model in the context of single cell barcode dropout.
...
It contains two data frames with the original data `dat` and the same data with NAs `na_dat`. `to_imp` is a list of 20 genes whose expression values have dropouts. The data contains a few thousand cells and 500 highly variable genes: 
...
`mice` generally discourages extracting the inner components of a `mids` object, but just this once:
